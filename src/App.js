...
import ImagesComponent from './ImagesContainer'
...
class App extends Component {
  ...
  playGame = id => {
    ...
    if (imagesList[imageIndex].id === id) {
      this.randomNumber = Math.ceil(Math.random() * imagesList.length)

      this.setState(prevState => ({
        score: parseInt(prevState.score) + 1,
        imageIndex: this.randomNumber,
      }))
    } else {
  ...
  componentDidMount = () => {
    this.timer = setInterval(this.changeSeconds, 1000)
  }
  ...
  render() {
    const {time, game, score, activeTabList, imageIndex} = this.state

    const filteredTabItems = imagesList.filter(
      eachItem => eachItem.category === activeTabList,
    )

    const imgContainerClassName =
      game === 'start' ? 'images-container' : 'block-img-container'

    const scoreCardClassName =
      game === 'over' ? 'score-card' : 'block-score-card'
      ...
      <div className="home-container">
        <nav>
          <ul className="nav-container">
            ...
            />
            <ul className="score-nav-container">
              <li className="score">
